program Test_VPI
  ...
  use vpi_gfn
  use vpi_potential, Usp_func => vpi_Usp_ho, &
                     Uij_func => vpi_Uij_hs, &
                     gUsp_func => vpi_gUsp_ho, &
                     gUij_func => vpi_gUij_hs, &
                     Uij_rot_func => vpi_Uij_dimer
  use vpi_trial_func, tfunc => ho_tfun, &
                      jas_tfun => hs_tfun, &
                      grad_lap_sp_tfun => grad_lap_ho_tfun, &
...
111   close(10)
    if(i .lt. N_SLICE) then
      ...
      call vpi_make_lattice( q0, size_x(1), DWELL_GAUSSIAN_FILL)
    end if
...
#ifdef USE_SWFN
    lntfn0 = 2.0_b8*( tfunc(q0, CSLICE, N_SLICE, N_PARTICLE, N_DIM) + jas_tfun(xij2_0, CSLICE, N_SLICE, N_PARTICLE) )
#else
