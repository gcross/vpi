...
!-----------------------------------------------------------------------!
 ...
 INTERFACE
  SUBROUTINE calcr(n,p,x,nf,r,nl2sol_iteration,been_rst,stop_opt,x_h)
   USE machine_constants
   IMPLICIT NONE
...
! vscopy... sets all elements of a vector to a scalar.
 ...
 REAL(dp) h,xk
 REAL(dp),PARAMETER :: hfac=1.d3,negpt5=-0.5d0,one=1.d0,zero=0.d0
...
10 nf=iv(nfcall)
 call calcr(n,p,x,nf,v(r1:r1+n-1),iv(31),iv(9),stop_nl2sol)
 if(strted)goto 20
 if(nf>0)goto 30
 iv(1)=13
 goto 90
...
!40 x(k)=xk+h
!  nf=iv(nfgcal)
!  call calcr(n,p,x,nf,v(j1k:j1k+n-1),uiparm,urparm,ufparm,iv(31),iv(9))
!  if(nf>0)goto 50
...
30 j1k=j1
 ...
 enddo
 nf=iv(nfgcal)
 call calcr(n,p,x,nf,v(j1:j1+n*p-1),iv(31),iv(9),stop_nl2sol,x_h)
 j1k=j1
 do k=1,p
  h=h_x(k)
  do i=r1,rn
...
REAL(dp),PARAMETER :: one=1.0d0,p01=0.01d0,p99=0.99d0,zero=0.0d0
